...
| Ticket | Description | Status | Updated_at | Note |
| --- | --- | --- | --- | --- |
| SC-070 | GoReleaser configuration (cross-compilation, GitHub Releases) | resolved | 2026-03-13 | .goreleaser.yml with 4 build targets (macOS arm64/amd64, Linux amd64, Windows amd64), ldflags version injection, checksums, changelog, Homebrew tap auto-update, Docker image publishing with build args. release.yml with lint+test gates. Review fixes: Docker version tag v-prefix, build args passed to Dockerfile. |
| SC-071 | Dockerfile for Docker image | resolved | 2026-03-13 | Multi-stage build: golang:1.23-alpine builder (CGO_ENABLED=1) → alpine:3.19 runtime with git + ca-certificates. Version injection via build args. DEFERRED: grammar bundling (requires grammar download infra). |
| SC-072 | Install script (`get.vettcode.com`) | resolved | 2026-03-13 | POSIX shell script: OS/arch detection with platform validation, GitHub Releases download, anchored SHA-256 checksum verification, sudo availability check, Windows .exe handling. Review fixes: Linux ARM64 guard, rate-limit hint, anchored grep. |
| SC-073 | Homebrew tap formula | resolved | 2026-03-13 | Auto-generated by GoReleaser (brews: section). Reference template in packaging/homebrew/. Installation: brew install vettcode/tap/vettcode. |
| SC-074 | Version check mechanism (24h-throttled, non-blocking, cached in ~/.vettcode/) | resolved | 2026-03-13 | internal/updater package: 24h cache throttling, 2s HTTP timeout, atomic cache writes (0600 perms), semver comparison, VETTCODE_NO_UPDATE_CHECK env var, silent failure. 18 tests. Review fixes: correct API endpoint, env var handling, fallback URL test, file permissions. |
