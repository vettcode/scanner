...
| SC-066 | Performance benchmarks setup | merged | 2026-03-14 | All benchmarks implemented. Full-scan benchmarks: 30K LOC ~600ms (target <2min), 100K LOC ~2s (target <5min), memory ~423MB (target <1GB). Synthetic 6-language repo generator with configurable LOC. |

## Epic 7: Distribution (0.5 days)

| Ticket | Description | Status | Updated_at | Note |
| --- | --- | --- | --- | --- |
| SC-070 | GoReleaser configuration (cross-compilation, GitHub Releases) | merged | 2026-05-07 | Single build with CGO overrides: linux/amd64 CGO=1 (full analysis), darwin and windows CGO=0 (stub — cannot cross-compile CGO from Linux without macOS SDK). complexity.go split into complexity_cgo.go (//go:build cgo) and complexity_nocgo.go (//go:build !cgo). Ubuntu runner; Docker on docker.io/vettcode/scanner and ghcr.io/vettcode/scanner. Homebrew tap auto-updated. v1.0.0 + v1.0.1 shipped. |
| SC-071 | Dockerfile for Docker image | merged | 2026-05-07 | Single-stage: debian:12-slim with git + ca-certs. GoReleaser provides the pre-built linux/amd64 binary; Dockerfile COPYs it in. debian:12-slim chosen for glibc compatibility with ubuntu runner (alpine/musl fails at runtime with CGO binary). |
| SC-072 | Install script (`get.vettcode.com`) | merged | 2026-05-07 | POSIX shell: platform validation, SHA-256 verification, sudo check, Windows .exe, rate-limit hint. Bug fixed: all function parameters declared `local` — verify_checksum() was overwriting global $archive, causing doubled temp path in tar extraction. Hosted on GCS bucket `get.vettcode.com` with Cloudflare SSL proxy. |
...
| SC-081 | Windows compatibility testing and fixes | merged | 2026-03-14 | CI matrix: ubuntu/windows/macos x Go 1.22/1.23. Platform-specific smoke tests (build + scan). Catches path separators, filepath behavior, ANSI codes. |
